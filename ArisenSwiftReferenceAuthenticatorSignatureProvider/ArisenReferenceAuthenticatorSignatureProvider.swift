//
//  ArisenReferenceAuthenticatorSignatureProvider.swift
//
...
/// Signature provider for Arisen SDK for Swift that provides selective disclosure and transaction signing using the
/// Arisen Reference iOS Authenticator App.  Conforms to `ArisenSignatureProviderProtocol`.
///
/// Requirements for developers.
/// + Add manifest.json file.
/// + Add URL scheme or Universal Link.
/// + Add URL scheme to whitelist.
public class ArisenReferenceAuthenticatorSignatureProvider: ArisenSignatureProviderProtocol {

    /// Singleton instance of the `ArisenReferenceAuthenticatorSignatureProvider`.
    public static let shared = ArisenReferenceAuthenticatorSignatureProvider()

    /// Map of the current completion closures for transaction signatures, keyed by the incoming payload ID.
    static var transactionSignatureCompletions = [String: ((ArisenTransactionSignatureResponse) -> Void)]()
    ...
    /// - seealso: https://github.com/ARISENIO/arisen-auth-transport-protocol-specification
    public struct RequestPayload: Codable {
        ...
        public init(hex: String) throws {
            guard let data = Data(hexString: hex) else {
                throw ArisenError(.signatureProviderError, reason: "Invalid request hex string")
            }
    ...
    public struct ResponsePayload: Codable {
        ...
        public init(hex: String) throws {
            guard let data = Data(hexString: hex) else {
                throw ArisenError(.signatureProviderError, reason: "Invalid response hex string")
            }
            print("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++")
        ...
        public init(id: String, transactionSignatureResponse: ArisenTransactionSignatureResponse) {
            self.id = id
            self.response.transactionSignature = TransactionSignatureResponse(arisenTransactionSignatureResponse: transactionSignatureResponse)
        }
