//
//  ArisenReferenceAuthenticatorSignatureProvider+SelectiveDisclosure.swift
...
/// Extensions to `ArisenReferenceAuthenticatorSignatureProvider` to add selective disclosure functionality.
extension ArisenReferenceAuthenticatorSignatureProvider {
    ...
    /// Request selective disclosure from the Arisen Reference iOS Authenticator App.
    /// Opens the Arisen Reference iOS Authenticator App and asks the user for permission.
    ///
    /// - Parameter request: The `SelectiveDisclosureRequest` being sent to the Arisen Reference iOS Authenticator App.
    ...
    public func requestSelectiveDisclosure(_ request: SelectiveDisclosureRequest, completion: @escaping (SelectiveDisclosureResponse) -> Void) {
        ...
        ArisenReferenceAuthenticatorSignatureProvider.selectiveDisclosureCompletions[payload.id] = completion
        guard let requestHex = payload.toHex else {
            return completion(SelectiveDisclosureResponse(error: ArisenError(.signatureProviderError, reason: "Unable to encode hex request")))
        }

        guard let url = URL(string: "Arisen://request?payload=\(requestHex)") else {
            return completion(SelectiveDisclosureResponse(error: ArisenError(.signatureProviderError, reason: "Unable to create url")))
        }
    ...
    /// Get the list of keys from the available authorizers.  Required to conform to `ArisenSignatureProviderProtocol`.
    ///
    /// - Parameter completion: Closure to call with the `ArisenAvailableKeysResponse`.
    public func getAvailableKeys(completion: @escaping (ArisenAvailableKeysResponse) -> Void) {
        getAuthorizers { (availableAuthorizersResponse) in
            var availableKeysResponse = ArisenAvailableKeysResponse()
    ...
    /// Request authorizers from the Arisen Reference iOS Authenticator App.
    /// Opens the Arisen Reference iOS Authenticator App and asks the user for permission.
    ///
    /// - Parameter completion: The completion closure to be called with the `SelectiveDisclosureResponse`.
    ...
    public func clearAuthorizers() throws {
        let url = try ArisenReferenceAuthenticatorSignatureProvider.authorizersURL()
    ...
    static func selectiveDisclosureURL() throws -> URL {
        guard let documentsDirectory = FileManager.default.urls(for: .documentDirectory, in: .userDomainMask).first else {
            throw ArisenError(.signatureProviderError, reason: "Cannot get document directory")
        }
